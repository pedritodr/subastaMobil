    ...
    function onDeviceReady() {
       ...
       /* window.FirebasePlugin.onNotificationOpen(function(notification) {
        ...
        setTimeout(function(){
            ...
            var ptienda_id = item_auth.tienda_id;
           
            var url = base_url + 'rest_user/load_operaciones_tienda';
            $.ajax
                ({
                    type: "POST",
                    url: url,
                    ...
                    data: {
                        ...
                        offset:0,
                        tienda_id:ptienda_id
                    },
                    success: function (response) {
                        
                        if (response.status == 200) {
                            $("#container_loading").hide();
                            var lista = response.lista_operaciones;
                            $("#lista_transacciones").empty();
                            offset_nuevo = response.offset;                            
                            pintar_transacciones(lista,response.show_next_page);
